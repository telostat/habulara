...
prepareSpec ss =
  ...
  where
    ...
    mkDescription x =
      "- Name:         "
        <> T.unpack (statField x)
        <> "\n"
        <> "- Type:         "
        <> show (statType x)
        <> "\n"
        <> "- Min:          "
        <> fromMaybe "#N/A" (statMin x)
        <> "\n"
        <> "- Max:          "
        <> fromMaybe "#N/A" (statMax x)
        <> "\n"
        <> "- Min Length:   "
        <> fromMaybe "#N/A" (statMinLength x)
        <> "\n"
        <> "- Max Length:   "
        <> fromMaybe "#N/A" (statMaxLength x)
        <> "\n"
        <> "- Stddev:       "
        <> fromMaybe "#N/A" (statStddev x)
        <> "\n"
        <> "- Mean:         "
        <> fromMaybe "#N/A" (statMean x)
        <> "\n"
        <> "- Median:       "
        <> fromMaybe "#N/A" (statMedian x)
        <> "\n"
        <> "- Cardinality:  "
        <> fromMaybe "#N/A" (statCardinality x)
        <> "\n"
